...
st.set_page_config(
    ...
    page_icon="✈️",
    layout="wide",
    initial_sidebar_state="collapsed",   # sidebar collapsed & hidden
)
...
:root {{
  --primary:#FF6B35; --accent:#FFD166; --sky:#5eb8ff; --teal:#2dd4bf;
  --success:#06D6A0; --bg-dark:#0A0E1A; --bg-card:#111827; --bg-card2:#1a2235;
  --text:#F8F9FA; --muted:#8899AA; --border:rgba(255,107,53,0.25);
  --gold:#d4a853; --gold-lt:#f0c96b; --radius:16px;
}}
...
.block-container {{ padding:1.2rem 2.5rem 4rem; max-width:1280px; }}
...
.topnav-brand {{
  ...
  font-size:1.6rem; font-weight:700; color:var(--gold); letter-spacing:-0.5px;
}}
.topnav-sub {{
  font-size:.62rem; color:rgba(240,236,228,.35);
  letter-spacing:3px; text-transform:uppercase; margin-top:1px;
}}
.topnav-status {{
  display:inline-flex; align-items:center; gap:6px;
  background:rgba(6,214,160,.12); border:1px solid rgba(6,214,160,.35);
  color:#06D6A0; border-radius:50px; padding:4px 12px;
...
.hero-sub {{
  font-size:0.75rem; color:rgba(240,236,228,0.4);
...
.weather-card {{
  ...
  border:1px solid rgba(0,78,137,0.4); border-radius:20px;
  padding:1.4rem; text-align:center; margin-bottom:1rem;
}}
.weather-temp {{ font-family:'Space Mono',monospace; font-size:3rem; font-weight:700; color:var(--accent); line-height:1; }}
.weather-city {{ font-family:'Playfair Display',serif; font-size:1.3rem; font-weight:700; color:var(--text); margin-bottom:4px; }}
.weather-meta {{ display:flex; justify-content:center; gap:1.2rem; margin-top:.8rem; font-size:.82rem; color:var(--muted); }}
.map-link {{
  display:inline-flex; align-items:center; gap:6px;
...
/* ── METRICS ── */
.metric-box   {{ background:var(--bg-card2); border:1px solid var(--border); border-radius:12px; padding:1rem; text-align:center; margin-bottom:.6rem; }}
.metric-val   {{ font-family:'Space Mono',monospace; font-size:1.8rem; font-weight:700; color:var(--accent); }}
.metric-label {{ font-size:.72rem; color:var(--muted); text-transform:uppercase; letter-spacing:.1em; margin-top:.15rem; }}
...
[data-testid="stExpander"] summary:hover {{ background:var(--bg-card2) !important; }}
[data-testid="stExpander"] summary p,
[data-testid="stExpander"] summary span,
[data-testid="stExpander"] summary div {{ color:white !important; }}
[data-testid="stExpander"] summary svg {{ fill:white !important; stroke:white !important; }}
...
label, .stMarkdown, .stMarkdown p, .stMarkdown div, .stMarkdown span, .stMarkdown li {{ color:white !important; }}
...
.stDownloadButton > button {{ color:black !important; font-weight:600 !important; }}
...
def get_weather(city: str) -> dict | None:
    ...
    try:
        ...
        return {
            ...
            "temp": round(res["main"]["temp"]), "feels_like": round(res["main"]["feels_like"]),
            "description": desc, "humidity": res["main"]["humidity"],
...
def get_travel_response(user_input: str, history: list, context: str = "") -> str:
    if not GROQ_API_KEY: return "⚠️ GROQ_API_KEY is not configured. Check your .env file."
    try:
        llm = _build_llm(temperature=0.7, max_tokens=1200)
        system_content = ("You are Wandr, a luxury travel concierge with deep expertise "
...
def get_itinerary(destination: str, days: int, budget: str,
    ...
    if not GROQ_API_KEY: return "⚠️ GROQ_API_KEY is not configured."
    try:
...
def index_pdf(file_bytes: bytes) -> list[str]:
    try:
        ...
        raw_pages = [p.extract_text() for p in reader.pages if p.extract_text() and p.extract_text().strip()]
...
def search_pdf_context(chunks: list[str], query: str, top_k: int = 3) -> str:
    ...
    scored = sorted([(sum(1 for w in query_words if w in c.lower()), c) for c in chunks], key=lambda x: -x[0])
...
# ══════════════════════════════════════════════════════════════════════════════
_defaults = {
    "chat_messages": [], "pdf_chunks": None, "pdf_page_count": 0,
    "current_place": None, "bg_url": random.choice(UNSPLASH_FALLBACK),
    "weather_data": None, "itinerary_result": None, "itinerary_dest": None,
    "history": [], "saved_itineraries": [],
}
...
<div class="topnav">
  <div>
    <div class="topnav-brand">✈ Wandr</div>
    <div class="topnav-sub">AI Travel Concierge</div>
  </div>
  <div style="display:flex;align-items:center;gap:12px;">
    {'<span class="status-badge status-pdf">📄 RAG Mode</span>' if st.session_state.pdf_chunks else
     '<span class="topnav-status"><span class="status-dot2"></span>Online & Ready</span>'}
    {'<span style="font-family:Playfair Display,serif;color:var(--gold-lt);font-style:italic;font-size:.95rem;">📍 ' + st.session_state.current_place.title() + '</span>' if st.session_state.current_place else ''}
    <span style="font-size:.7rem;color:var(--muted);">{len(st.session_state.history)} searches · {len(st.session_state.saved_itineraries)} saved</span>
  </div>
</div>
""", unsafe_allow_html=True)
 
# ══════════════════════════════════════════════════════════════════════════════
# CONTROL PANEL — replaces sidebar, shown as expander below nav
# ══════════════════════════════════════════════════════════════════════════════
with st.expander("⚙️  Controls — Quick Destinations · Weather · PDF Upload · Session", expanded=False):
    cp1, cp2, cp3, cp4 = st.columns([1.2, 1.2, 1.5, 1])
 
    # ── QUICK DESTINATIONS ────────────────────────────────────────────────────
    with cp1:
        st.markdown('<div class="panel-section-title">🎯 Quick Destinations</div>', unsafe_allow_html=True)
        quick_cities = [
        ...
        row3 = st.columns(2)
        rows = [row1, row2, row3]
        for i, (label, city_key) in enumerate(quick_cities):
            with rows[i // 2][i % 2]:
                if st.button(label, key=f"quick_{city_key}", use_container_width=True):
                    ...
                    if GROQ_API_KEY:
                        ...
                        st.session_state.history.append({"query": prompt, "response": reply,
                                                          "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M")})
                    st.rerun()
 
    # ── QUICK WEATHER ─────────────────────────────────────────────────────────
    with cp2:
        st.markdown('<div class="panel-section-title">🌍 Quick Weather</div>', unsafe_allow_html=True)
        quick_city = st.text_input("City name", placeholder="e.g. Tokyo", key="panel_city", label_visibility="collapsed")
        if st.button("🌤️ Check Weather", use_container_width=True, key="panel_wx_btn"):
            if not WEATHER_API_KEY:
                st.warning("⚠️ Add WEATHER_API_KEY")
        ...
        if st.session_state.weather_data:
            ...
            st.markdown(f"""
<div style="background:rgba(0,78,137,.2);border:1px solid rgba(0,78,137,.4);border-radius:10px;padding:10px;text-align:center;margin-top:6px;">
  <div style="font-size:1.8rem;">{w['icon']}</div>
  <div style="font-family:'Playfair Display',serif;font-weight:700;color:white;">{w['city']}</div>
  <div style="font-family:'Space Mono',monospace;font-size:1.6rem;color:var(--accent);font-weight:700;">{w['temp']}°C</div>
  <div style="font-size:.75rem;color:var(--muted);">{w['description'].title()}</div>
  <div style="font-size:.72rem;color:var(--muted);margin-top:4px;">💧{w['humidity']}% · 💨{w['wind']}km/h</div>
</div>""", unsafe_allow_html=True)
 
    # ── PDF UPLOAD ────────────────────────────────────────────────────────────
    with cp3:
        st.markdown('<div class="panel-section-title">📄 PDF Document (RAG)</div>', unsafe_allow_html=True)
        ...
        if st.session_state.pdf_chunks:
            st.markdown(f'<span class="status-badge status-pdf">📄 PDF Loaded — {st.session_state.pdf_page_count} chunks</span>', unsafe_allow_html=True)
    ...
    # ── SESSION STATS + CLEAR ─────────────────────────────────────────────────
    with cp4:
        st.markdown('<div class="panel-section-title">📊 Session</div>', unsafe_allow_html=True)
        ...
        if st.button("🧹 Clear All", use_container_width=True):
            for k in ["chat_messages","pdf_chunks","current_place","weather_data","itinerary_result","itinerary_dest","history"]:
                st.session_state[k] = [] if k in ["chat_messages","history"] else None
            st.session_state.bg_url = random.choice(UNSPLASH_FALLBACK)
            st.rerun()
 
# ══════════════════════════════════════════════════════════════════════════════
# DESTINATION BANNER + MODE BADGE + PILLS
# ══════════════════════════════════════════════════════════════════════════════
  ...
  <div style="font-size:.88rem;margin-top:.5rem;">Ask about destinations, packing, visas, budgets — or upload a PDF to chat with it via RAG.</div>
        ...
        if send_btn and user_input.strip():
            if not GROQ_API_KEY:
                ...
                context = ""
                if st.session_state.pdf_chunks:
                    context = search_pdf_context(st.session_state.pdf_chunks, user_input)
                with st.spinner("✈️ Crafting your answer…"):
                    reply = get_travel_response(user_input, st.session_state.chat_messages[:-1], context=context)
                st.session_state.chat_messages.append({"role": "assistant", "content": reply})
                st.session_state.history.append({"query": user_input, "response": reply,
                                                  "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M")})
                cursor.execute("INSERT INTO searches(query,response,timestamp) VALUES(?,?,?)",
...
with tab_itin:
    ...
    with col_form:
        ...
        cb, cs = st.columns(2)
        with cb: budget = st.selectbox("💰 Budget", ["Mid-range", "Luxury"])
        with cs: style  = st.selectbox("🎭 Style", ["Culture & History","Adventure","Food & Nightlife","Relaxation","Family","Backpacker"])
    ...
    with col_result:
        if gen_btn:
            if not GROQ_API_KEY:
                ...
                with st.spinner(f"✈️ Crafting your {days}-day {destination} adventure…"):
                ...
                st.session_state.history.append({
                    ...
                    "response": itin, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"),
                })
            ...
            with cs1:
                if st.button("💾 Save Itinerary", use_container_width=True):
                    st.session_state.saved_itineraries.append({
                        ...
                        "days": days, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M"),
                    })
                    st.success("✅ Saved!")
            with cs2:
                if st.button("🗑️ Clear", use_container_width=True, key="clear_itin"):
                    ...
                    st.rerun()
            st.download_button(label="📥 Download Itinerary", data=st.session_state.itinerary_result,
                               file_name="travel_itinerary.txt", mime="text/plain", use_container_width=True)
        else:
            st.markdown("""
...
with tab_weather:
        ...
        for lbl, city_name in popular:
            if st.button(lbl, key=f"pop_{city_name}", use_container_width=True):
                if not WEATHER_API_KEY:
                    ...
                    with st.spinner(f"Fetching {city_name}…"):
    ...
    with cw2:
        if search_weather and city_input.strip():
            if not WEATHER_API_KEY:
                st.error("WEATHER_API_KEY not set!")
            else:
...
""", unsafe_allow_html=True)
            details = [
                ("🌡️","Feels Like",f"{w['feels_like']}°C"),("💧","Humidity",f"{w['humidity']}%"),
                ("💨","Wind Speed",f"{w['wind']} km/h"),("📊","Pressure",f"{w['pressure']} hPa"),
                ("☁️","Cloudiness",f"{w['clouds']}%"),("👁️","Visibility",f"{w['visibility']} km"),
            ]
            cols3 = st.columns(3)
...
<div class="card" style="text-align:center;padding:3rem;">
  ...
  <div style="color:#8899AA;font-size:.83rem;margin-top:.4rem;">Powered by OpenWeatherMap API</div>
</div>""", unsafe_allow_html=True)
