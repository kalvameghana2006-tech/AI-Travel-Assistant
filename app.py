...
Setup
-----
1. Create a `.env` file (or set Streamlit secrets) with:
       GROQ_API_KEY=<your_groq_key>
       WEATHER_API_KEY=<your_openweathermap_key>
2. Install dependencies:
       pip install streamlit langchain langchain-groq langchain-core \
                   langchain-text-splitters PyPDF2 requests python-dotenv
3. Run locally:
       streamlit run app.py
4. Deploy: push repo to GitHub → connect on share.streamlit.io
"""
...
from langchain_groq import ChatGroq          # ← LangChain + Groq integration
from langchain_core.messages import (        # ← LangChain message schema
    HumanMessage,
    SystemMessage,
    AIMessage,
)
from langchain_text_splitters import (       # ← LangChain RAG text splitting
...
# Support both .env file and Streamlit Secrets (for Streamlit Cloud)
...
# ══════════════════════════════════════════════════════════════════════════════
# ── BACKEND: OPENWEATHERMAP API  (uses `requests` library) ───────────────────
# ══════════════════════════════════════════════════════════════════════════════
def get_weather(city: str) -> dict | None:
    """Fetch live weather from OpenWeatherMap REST API using `requests`."""
    ...
    try:
        ...
        res = requests.get(url, timeout=6).json()   # ← requests.get()
        if res.get("cod") != 200:
            return None
...
# ══════════════════════════════════════════════════════════════════════════════
# ── BACKEND: LANGCHAIN + GROQ  (LLM calls) ───────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
def _build_llm(temperature: float = 0.7, max_tokens: int = 1200) -> ChatGroq:
    """Construct a LangChain ChatGroq LLM instance."""
    return ChatGroq(                         # ← LangChain + Groq
...
def detect_city_from_text(text: str) -> str | None:
    """Use Groq LLM via LangChain to extract a city name from free text."""
    if not GROQ_API_KEY:
        return None
    try:
        ...
        result = llm.invoke(messages)        # ← LangChain .invoke()
...
def get_travel_response(user_input: str, history: list, context: str = "") -> str:
    """
    Main chat handler — uses LangChain ChatGroq.
    If `context` is provided (RAG), answers are grounded in that text.
    """
    if not GROQ_API_KEY:
    ...
    try:
        ...
        response = llm.invoke(messages)      # ← LangChain .invoke()
...
def get_itinerary(destination: str, days: int, budget: str,
    ...
    """Generate a detailed travel itinerary using LangChain + Groq."""
    if not GROQ_API_KEY:
...
# ══════════════════════════════════════════════════════════════════════════════
# ── RAG: DOCUMENT PROCESSING  (PyPDF2 + LangChain text splitter) ─────────────
# ══════════════════════════════════════════════════════════════════════════════
def index_pdf(file_bytes: bytes) -> list[str]:
    """
    RAG Step 1 — Document Ingestion using PyPDF2.
    Reads each page and splits into overlapping chunks
    via LangChain's RecursiveCharacterTextSplitter.
    """
    try:
        reader = PyPDF2.PdfReader(io.BytesIO(file_bytes))  # ← PyPDF2
        raw_pages: list[str] = []
        ...
        splitter = RecursiveCharacterTextSplitter(   # ← LangChain RAG
            chunk_size=800,
            chunk_overlap=100,
            separators=["\n\n", "\n", ".", " ", ""],
        )
...
def search_pdf_context(chunks: list[str], query: str, top_k: int = 3) -> str:
    """
    RAG Step 2 — Retrieval: keyword-scored chunk selection.
    Ranks chunks by query-word overlap and returns the top-k most relevant.
    This is a lightweight RAG retriever (no vector DB needed for Streamlit Cloud).
    """
    if not chunks:
        return ""
...
_defaults = {
    "chat_messages":      [],
    "pdf_chunks":         None,   # RAG: split document chunks
    "pdf_page_count":     0,
    "current_place":      None,
...
""", unsafe_allow_html=True)
    ...
    # ── PDF UPLOAD  (PyPDF2 + LangChain RAG) ─────────────────────────────────
    ...
    if uploaded_file and st.session_state.pdf_chunks is None:
        with st.spinner("📄 Indexing PDF with PyPDF2 + LangChain RAG…"):
            chunks = index_pdf(uploaded_file.read())   # ← PyPDF2 + LangChain splitter
            if chunks:
    ...
    # ── QUICK WEATHER (OpenWeatherMap API) ────────────────────────────────────
    ...
    if st.button("🌤️ Check Weather", use_container_width=True):
        if not WEATHER_API_KEY:
            ...
            with st.spinner("Fetching from OpenWeatherMap…"):
                wdata = get_weather(quick_city)     # ← OpenWeatherMap + requests
            if wdata:
        ...
        if st.button(label, use_container_width=True, key=f"quick_{city_key}"):
            ...
            if GROQ_API_KEY:
                with st.spinner("Planning your trip…"):
                    reply = get_travel_response(prompt, [])   # ← LangChain + Groq
  ...
  <div style="font-size:.88rem;margin-top:.5rem;">Ask about destinations, packing, visas, budgets — or upload a PDF to chat with it via RAG.</div>
        ...
        if send_btn and user_input.strip():
            if not GROQ_API_KEY:
                ...
                # ── RAG retrieval (PyPDF2 chunks → keyword search) ────────────
                context = ""
                if st.session_state.pdf_chunks:
                    context = search_pdf_context(
                        st.session_state.pdf_chunks, user_input   # ← RAG retrieval
                    )

                with st.spinner("✈️ Crafting your answer with Groq LLM…"):
                    reply = get_travel_response(     # ← LangChain + Groq
                        user_input,
                        st.session_state.chat_messages[:-1],
                        context=context,             # ← RAG context injection
                    )
...
with tab_itin:
    ...
    with col_form:
        ...
        if WEATHER_API_KEY and destination.strip():
            wdata = get_weather(destination.split(",")[0])   # ← OpenWeatherMap
            if wdata:
...
<div style="background:rgba(255,209,102,.08);border:1px solid rgba(255,209,102,.2);border-radius:8px;padding:.65rem;font-size:.83rem;margin-top:.4rem;">
  {wdata['icon']} Live weather: <b style="color:#FFD166;">{wdata['temp']}°C</b>, {wdata['description'].title()}
    ...
    with col_result:
        # ── FIX: generate here, inside col_result, NO st.rerun() ─────────────
        if gen_btn:
            if not GROQ_API_KEY:
                ...
                # ── NO st.rerun() here — result renders immediately below ──
            ...
            if st.button(lbl, key=f"pop_{city_name}", use_container_width=True):
                if not WEATHER_API_KEY:
                    ...
                    with st.spinner(f"Fetching {city_name} weather from OpenWeatherMap…"):
                        wdata = get_weather(city_name)       # ← requests + OpenWeatherMap
                    if wdata:
                        st.session_state.weather_data  = wdata
                        st.session_state.bg_url        = get_city_image(city_name)
                        st.session_state.current_place = city_name
                        # ── NO st.rerun() — renders immediately in cw2 below ──
                    else:
                        st.error("Could not fetch weather")

    with cw2:
        # ── FIX: search also handled here, NO st.rerun() ─────────────────────
        if search_weather and city_input.strip():
            if not WEATHER_API_KEY:
                ...
                with st.spinner("Fetching weather data from OpenWeatherMap API…"):
                    wdata = get_weather(city_input)          # ← requests + OpenWeatherMap
                if wdata:
                    st.session_state.weather_data  = wdata
...
<div class="metric-box">
  ...
  <div class="metric-label">{label}</div>
            ...
            # AI travel advice using LangChain + Groq
            if GROQ_API_KEY and st.button("🤖 Get AI Travel Advice for This Weather", use_container_width=True):
                with st.spinner("Generating advice with Groq LLM…"):
                    advice = get_travel_response(   # ← LangChain + Groq
